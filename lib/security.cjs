...
function normalizeDefectItem(item, fallbackProduct = "未分類", options = {}) {
  ...
  const maxFutureMs = Math.max(0, Number(options.maxFutureMs || 0));
  const maxPastMs = Math.max(0, Number(options.maxPastMs || 0));
  if (maxFutureMs > 0 && timestamp.getTime() - now.getTime() > maxFutureMs) {
    throw new Error("timestamp 超過允許的未來時間範圍");
  }
  if (maxPastMs > 0 && now.getTime() - timestamp.getTime() > maxPastMs) {
    throw new Error("timestamp 超過允許的歷史時間範圍");
  }

  return { id, status, product, image_url, timestamp };
}

function normalizeDefectPayload(payload, fallbackProduct = "未分類", options = {}) {
  ...
  if (Array.isArray(payload.items)) {
    ...
    return {
      system_id,
      items: payload.items.map(item => normalizeDefectItem(item, fallbackProduct, options))
    };
  }

  return { system_id, items: [normalizeDefectItem(payload, fallbackProduct, options)] };
}
